    ...
    try:
        ...
        try:
            ...
            for row in brand_results:
                brand_val, section_val, product_val, model_val, item_val = row[0], row[1], row[2], row[3], row[4]
        ...
        logger.info(f"Response data - r_scores: {r_scores_final[:3] if r_scores_final else 'empty'}, types: {[type(x).__name__ for x in r_scores_final[:3]] if r_scores_final else 'empty'}")
        logger.info(f"Response data - brand_hierarchy sample: {brand_hierarchy_final[:2] if brand_hierarchy_final else 'empty'}")
        ...
        try:
            ...
            if hasattr(validation_error, 'errors'):
                ...
                for err in error_details:
                    ...
                    logger.error(f"  Field: {field_path}, Error: {err.get('msg')}, Input: {err.get('input')}")
            logger.error(f"r_scores_clean: {r_scores_clean} (types: {[type(x).__name__ for x in r_scores_clean[:5]] if r_scores_clean else 'empty'})")
            logger.error(f"f_scores_clean: {f_scores_clean} (types: {[type(x).__name__ for x in f_scores_clean[:5]] if f_scores_clean else 'empty'})")
            logger.error(f"m_scores_clean: {m_scores_clean} (types: {[type(x).__name__ for x in m_scores_clean[:5]] if m_scores_clean else 'empty'})")
            logger.error(f"brand_hierarchy_final sample: {brand_hierarchy_final[:2] if brand_hierarchy_final else 'empty'}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error loading campaign options: {error_msg}"
        )
