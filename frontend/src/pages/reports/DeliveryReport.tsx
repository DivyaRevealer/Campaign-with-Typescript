...
import { getDeliveryReport, type DeliveryReportResponse } from "@/api/deliveryReports";
...
export default function DeliveryReport() {
  ...
  const totalQuantity = useMemo(() => {
    ...
    const parsedQuantities = report.items.map((item) => Number(item.dely_qty));
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const data = await getDeliveryReport(query);
      setReport(data);
    } catch (error) {
      setReport(null);
      if (isAxiosError(error)) {
        ...
        if (typeof detail === "string" && detail.trim()) {
          ...
          setApiError("Unable to load the delivery report. Please try again.");
        }
      } else {
        setApiError("Unable to load the delivery report. Please try again.");
      }
  ...
  const handleExport = () => {
    ...
    const rows = report.items.map((item) => [
      item.description ?? "",
      item.part_no ?? "",
      formatDate(item.dely_date),
      formatQuantity(item.dely_qty),
    ]);

    const safeFileName = `${sanitizeFileName(report.so_no, "delivery-report")}.xlsx`;
    const sheetName = sanitizeSheetName(report.so_no, "Delivery");

    exportToExcel({ header, rows, fileName: safeFileName, sheetName });
  };

  return (
    <div className="admin-page delivery-report-page">
      <header className="delivery-report__header">
        <div className="delivery-report__header-content">
          <h1>Delivery Report</h1>
          <p>Look up delivery details by entering a Sales Order No.</p>
        </div>
        <div className="delivery-report__header-actions">
          <ExportMenu onSelectExcel={handleExport} disabled={!canExport} />
        </div>
      </header>

      <form className="delivery-report__search-form" onSubmit={onSubmit} role="search" aria-label="Delivery report search">
        <div className={`admin-search${soNo ? " admin-search--has-value" : ""}`}>
          <label className="visually-hidden" htmlFor="delivery-report-search">
            Sales Order No
          </label>
      ...
      {report && report.items.length > 0 && (
        <div className="delivery-report__summary" role="status">
          <span>
      ...
      <div className="admin-table-wrapper" aria-live="polite">
        {loading && <div className="delivery-report__status">Fetching delivery details…</div>}

        {!loading && hasRequested && report && report.items.length === 0 && (
          <div className="delivery-report__status">
            No delivery records found for <strong>{report.so_no}</strong>.
          </div>
        )}

        {!loading && !report && !apiError && !validationError && !hasRequested && (
          <div className="delivery-report__status">
            Enter a Sales Order No above to view the delivery report.
          </div>
        )}
          ...
          <table className="admin-table">
            ...
            <tbody>
              {report.items.map((item, index) => (
                <tr key={`${item.description}-${item.part_no ?? ""}-${item.dely_date ?? index}`}>
                  <td>{item.description}</td>
                  <td style={{ whiteSpace: "nowrap" }}>{item.part_no || ""}</td>
                  <td style={{ whiteSpace: "nowrap" }}>{formatDate(item.dely_date)}</td>
                  <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>{formatQuantity(item.dely_qty)}</td>
                </tr>
              ))}
            </tbody>
          </table>
