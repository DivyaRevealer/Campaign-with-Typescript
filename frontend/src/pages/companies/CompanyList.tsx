...
import { fetchCompanies, type Company, type CompanyListResponse } from "../../api/companies";
...
export default function CompanyList() {
  ...
  const load = useCallback(
    async (p = 0, search = q) => {
      ...
      const { items, total }: CompanyListResponse = await fetchCompanies({
        q: query,
        limit,
        offset: p * limit,
      });
      ...
      <div className="admin-table-wrapper">
        <table className="admin-table">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.comp_code}>
                <td style={{ whiteSpace: "nowrap" }}>{r.comp_code}</td>
                <td>{r.comp_name}</td>
                <td>{r.comp_city || ""}</td>
                <td>{r.comp_state || ""}</td>
                <td style={{ whiteSpace: "nowrap" }}>{r.comp_contact_no || ""}</td>
                <td>{r.comp_email || ""}</td>
                <td>{statusPill(r.active_flag)}</td>
                <td>
                  <Link className="table-link" to={`/companies/${encodeURIComponent(r.comp_code)}`}>
                    Edit
                  </Link>
                </td>
              </tr>
